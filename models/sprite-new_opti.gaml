...
species dyke parent: obstacle
{
	...
	aspect triangle {
		...
		draw triangle(150) color: couleur;
	}
...
grid parcelle width: 52 height: 90 neighbours: 8 frequency: 0 use_regular_agents: false use_individual_shapes: false use_neighbours_cache: false
{
   ...
   float distanceSea;
	...
	//bool est_ecolo<-false;
        // which territory does the cell belong to
	// TODO : choose it from a shapefile
	...
	//bool maison <- false;
	//bool maison update: !empty(agents_overlapping(self) of_species building);
	...
	//valeur securite
	float valeurSecurite <- updateValeurSecurite() update: updateValeurSecurite();
	...
	float distanceSea  {
		...
		if (!empty(seaCells)) {
			parcelle closestSea <- (seaCells closest_to(self));  
...
// la satisfaction des habitants de cette parcelle est la somme des 3 indices (secu, ecolo, attractivite)
	...
	float updateValeurSecurite //when: !phase_sub
	{
		float secuDigue <- 0.0;
		// liste des digues
		list<dyke> digues <- dyke overlapping(self);
		
		// si aucune digue : securite 0
		if (empty(digues)) {secuDigue <- 0.0;}
		// si une ou plusieurs digues
		else {
			// s'il y a une vraie digue non ecolo : securite max
			if (!empty(digues where !each.est_ecolo)) {secuDigue <- 10.0;}
			// sinon (digues ecolos / brise lame : securite moyenne
			else {secuDigue <- 5.0;}
		}

		write 'distance to sea = '+distanceSea();
		write 'distance digue = '+distanceDigue();

		// distanceSea et distanceDigue influencent
		if (distanceSea() < 10) {secuDigue <- secuDigue-1;}
		if (distanceDigue() < 10) {secuDigue <- secuDigue +1 ;}
		
		//write("coucouc "+secuDigue );

		return secuDigue;
		//valeurSecurite <- secuDigue;
		// valeur agregee de la securite		
	...
	/*************************************
	 * raccourcis pour gestion du budget *
	 *************************************/
	...
	action submerge {
		// TODO: Franck, a remplacer par ton action
	}
	...
	user_command "construire une digue non ecolo"
	{ 
	...
	// 
	user_command "construire un brise lame (protection ecolo)"
	{
		create dyke {
			est_ecolo <- true;
			location <- myself.location;
...
//Definition de quelques parametres
	...
	output
	{
		...
		{
			chart "Cote de popularite du maire" type: histogram
			{
		...
		{
			chart "Niveau de securite du territoire" type: histogram
			{
