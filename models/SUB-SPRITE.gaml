...
*/
	...
	//boucle de submersion
		...
		if (eau_present) {
				...
				color <-#blue;
				dommage<-dommage+densite_bati;
				}
				if dommageTotal<dommage {dommageTotal<-dommage;}
		 	}
	...
	user_command PreparerNouveauTour {
		write "Passer au prochain Tour";
		dommageTotal<-0.0;
			ask (parcelle) {
	 			color <- # white;
		if (is_sea) {color <- # blue;}
...
grid parcelle width: 52 height: 90 neighbours: 8 frequency: 0 use_regular_agents: false use_individual_shapes: false use_neighbours_cache: false
{
   ...
   float distanceSea;
	...
	int digue <- 0;
	//bool ttest0<- !empty(dyke in agents_overlapping(self));
...
// la satisfaction des habitants de cette parcelle est la somme des 3 indices (secu, ecolo, attractivite)
	...
	action releveStatSubm {
		if water_height>0 {eau_present<-true;}
